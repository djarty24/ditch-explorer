	...
	const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === 'Enter') {
			...
			const newHistory: HistoryLine[] = [
				...history,
				{ id: Date.now(), type: 'input', text: `C:\\WINDOWS> ${trimmedInput}` }
			];
			...
			switch (cmd) {
				case 'help':
					newHistory.push({ id: Date.now() + 1, type: 'output', text: 'Available commands: help, clear, echo' });
					break;
				case 'clear':
			...
			<div className="bg-black h-80 p-2 font-mono text-sm overflow-y-auto" onClick={() => document.getElementById('cli-input')?.focus()}>
				{history.map((line) => (
					<div key={line.id} className={`mb-1 ${line.type === 'error' ? 'text-win-hotpink font-bold' : 'text-gray-300'}`}>
						{line.type === 'input' ? line.text : <TypewriterText text={line.text} />}
					</div>
				))}

				<div className="flex text-gray-300 mt-1">
					<span className="mr-2">C:\WINDOWS&gt;</span>
					<input
						id="cli-input"
						type="text"
						value={input}
