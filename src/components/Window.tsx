...
export default function Window({ title, content, hints, onClose, positionClass }: WindowProps) {
  ...
  // Default to top-left if no specific position is provided
  const posClass = positionClass || "top-10 left-10";

  return (
	<Draggable nodeRef={nodeRef} handle=".window-handle" bounds="parent">
	  <div ref={nodeRef} className={`absolute ${posClass} w-96 bg-win-gray border-t-2 border-l-2 border-white border-b-black border-r-black shadow-[2px_2px_0px_rgba(0,0,0,0.5)] flex flex-col z-40`}>
