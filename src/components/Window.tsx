...
interface WindowProps {
	...
	zIndex?: number;       // <-- NEW: Controls rendering order
	onFocus?: () => void;  // <-- NEW: Detects clicks
}

export default function Window({ title, content, hints, onClose, positionClass, zIndex, onFocus }: WindowProps) {
	...
	const posClass = positionClass || "top-10 left-10";

	return (
		<Draggable nodeRef={nodeRef} handle=".window-handle" bounds="parent">
			<div
				ref={nodeRef}
				onMouseDown={onFocus} // <-- Brings to front when clicked
				style={{ zIndex: zIndex || 40 }} // <-- Applies the dynamic layer order
				className={`absolute ${posClass} w-96 bg-win-gray border-t-2 border-l-2 border-white border-b-black border-r-black shadow-[2px_2px_0px_rgba(0,0,0,0.5)] flex flex-col`}
			>
